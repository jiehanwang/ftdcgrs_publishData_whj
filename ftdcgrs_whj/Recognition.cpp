...
void  CRecognition::BatchTest(CString TestFile, CString dir, int testID, int subID)
{
	...
	sprintf(filename, "%s%s%02d_%d%s", dir, "result_", testID, subID ,".txt");
	if ((fp3 = fopen(filename, "w")) == NULL) {
		AfxMessageBox("Can not create result file, return.");
		fclose(fp1);
		return;
	...
	sprintf(filename, "%s%s%02d_%d%s", dir, "result_detail_", testID, subID ,".txt");
	...
	for (int j=0; j<count; j++)
	{
		cout<<"P"<<testID<<"_"<<subID<<" sign ID: "<<j<<endl;
		fscanf(fp1, "%s", word);
		sprintf(filename, "%s%s%02d_%d%s%s%s", dir, "test_", testID, subID ,"\\", word, ".txt");
		int nWordNum, nRecogNum;
